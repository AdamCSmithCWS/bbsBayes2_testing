...
parameters {
 ...
 vector[n_strata] strata_raw;
  ...
  vector[n_years] YEAREFFECT_raw;
  
  vector[n_observers] obs_raw;    // observer
  vector[n_sites] ste_raw;   //
  real<lower=0.0001> sdnoise;    // sd of over-dispersion
  real<lower=0.0001> sdobs;    // sd of observer effects
  real<lower=0.0001> sdste;    // sd of site effects
  //array[n_knots_year] real<lower=0.0001> sdbeta;    // sd of GAM coefficients among strata
  real<lower=0.0001> sdbeta;    // sd of GAM coefficients among strata - one value across all k and strata
  real<lower=0.0001> sdstrata;    // sd of intercepts
  real<lower=0.0001> sdBETA;    // sd of GAM coefficients
  real<lower=0.0001> sdyear;    // sd of year effects
  real<lower=0.0001> sdYEAR;    // sd of survey-wide year effects
...
for(y in 1:n_years){
    yeareffect[,y] = sdyear*yeareffect_raw[,y] + YEAREFFECT[y];
...
model {
  ...
  sdyear ~ student_t(3,0,1); // prior on sd of yeareffects - stratum specific, prior placing 99% of prior mass at values < 1.2
  sdYEAR ~ student_t(3,0,1); // prior on sd of yeareffects - stratum specific, prior placing 99% of prior mass at values < 1.2
  // sdyear ~ ~ gamma(2,4); // prior on sd of yeareffects - stratum specific, and boundary-avoiding with a prior mode at 0.5 (1/2) - recommended by https://doi.org/10.1007/s11336-013-9328-2
 ...
 for(y in 1:n_years){
  ...
  yeareffect_raw[,y] ~ icar_normal(n_strata, node1, node2);
